...
export async function GET(req: Request) {
  try {
    ...
    // Ensure baseUrl does not have a trailing slash to prevent double slashes in paths
    const baseUrl = (
    ...
    const logoUrl = activeLogo?.imageUrl
      ? urlFor(activeLogo.imageUrl).width(160).url()
          ...
          <body style="margin: 0; padding: 0; background-color: #ffffff; font-family: 'Times New Roman', Times, serif; display: flex; align-items: center; justify-content: center; height: 100vh;">
            <div style="text-align: center; border: 1px solid #f0f0f0; padding: 60px; max-width: 400px;">
              <img src="${logoUrl}"
                   ...
                   priority
                   height="auto"
                   style="max-width: 120px; height: auto; display: block; margin: 0 auto 30px auto; border: 0;" />
    ...
    // Get active logo from Sanity for error page
    const activeLogo = await getActiveLogo();
    const logoUrl = activeLogo?.imageUrl
      ? urlFor(activeLogo.imageUrl).width(100).url()
      ...
      `<html>
        <body style="font-family: 'Times New Roman', serif; background: #ffffff; color: #000; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0;">
          <div style="text-align: center; max-width: 400px; padding: 40px; border: 1px solid #f0f0f0;">
            <img src="${logoUrl}" alt="Logo" width="100" quality="100" priority style="margin-bottom: 30px;"/>
            <h2 style="color: #be1e2d; text-transform: uppercase; letter-spacing: 0.2em; font-size: 14px;">Process Error</h2>
            <p style="color: #666; font-size: 14px; margin: 20px 0;">We were unable to process your request at this time.</p>
            <a href="/" style="color: #000; text-decoration: none; border-bottom: 1px solid #000; font-size: 10px; letter-spacing: 0.2em; text-transform: uppercase;">Back to Store</a>
          </div>
        </body>
      </html>`,
      {
