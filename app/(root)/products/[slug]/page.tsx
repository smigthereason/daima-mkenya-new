...
export async function generateMetadata({
  ...
  const name = product.name;
  // ✅ Use categories array (first entry) instead of category
  const category = product.categories?.[0] || "Collection";
...
export default async function Page({ params }: PageProps) {
  ...
  // ✅ Build category string for JSON‑LD from categories array (or fallback)
  const categoryForJsonLd = product?.categories?.join(" / ") || "Fashion";
    ...
    product && !product.disabled
      ? {
          ...
          description:
            ...
            "Premium Kenyan-branded clothing made in Nairobi.",
          sku: product._id,
          category: categoryForJsonLd, // ✅ uses categories array
          brand: {
            "@type": "Brand",
            name: "Daima Mkenya Africa",
          },
