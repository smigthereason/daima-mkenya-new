    ...
    <section className="bg-[#e8e8e8] py-24 overflow-hidden border-b border-neutral-300">
        ...
        {essentials.map((item, idx) => (
          <div key={idx} className="group relative aspect-16/10 md:aspect-square lg:aspect-3/4 bg-neutral-900 flex flex-col justify-center items-center group overflow-hidden">
             ...
             <div className="relative z-10 text-center">
                {/* <span className="text-[10px] tracking-[0.4em] text-white/70 mb-3 block">{item.subtitle}</span> */}
                <h3 className="text-xl md:text-3xl font-serif tracking-[0.2em] text-white uppercase">{item.title}</h3>
             </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="text-[18px] tracking-[0.6em] text-neutral-800 uppercase">The Heritage Collection</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {secondaryGallery.map((item, i) => (
            <div key={i} className={`relative overflow-hidden transition-all duration-1000 ${i === 1 ? 'md:translate-y-20' : ''}`}>
      ...
      <div className="max-w-3xl mx-auto text-center px-6 mt-64 mb-20">
        <p className="text-xl md:text-2xl text-neutral-800 font-light leading-relaxed italic font-serif">
          "Identity is a story best worn. Born in Kenya, destined for the world."
        </p>
