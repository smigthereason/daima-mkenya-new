...
//       className={`
//         bg-transparent border-b border-black outline-none text-[10px] tracking-widest transition-all duration-500 ease-in-out
//         ${isSearchOpen ? "w-32 md:w-48 opacity-100 pr-8" : "w-0 opacity-0 pointer-events-none"}
//       `}
//     />
//     <Search 
//       size={24} 
...
//   {/* Your existing User/Profile Link */}
//   <Link href={session ? "/profile" : "/login"}>
//     {session?.user?.image ? (
//       <div className="w-7 h-7 relative rounded-full overflow-hidden border border-black/10">
//         <Image src={session.user.image} alt="User" fill className="object-cover" unoptimized={session.user.image.includes('ui-avatars.com')} />
//       </div>
//     ) : (
//       <User size={32} className="cursor-pointer hover:scale-110 transition-transform stroke-[1.5px] text-neutral-700" />
//     )}
//   </Link>
// </div>
//       </div>
...
const SearchLogic = ({ isSearchOpen, setIsSearchOpen, searchQuery, setSearchQuery, handleSearchSubmit }: any) => {
  ...
  return (
    <form onSubmit={handleSearchSubmit} className="relative flex items-center group">
      <input
        ...
        onChange={(e) => setSearchQuery(e.target.value)}
        onBlur={handleBlur}
        className={`bg-transparent border-b border-black outline-none text-[10px] tracking-widest transition-all duration-500 ease-in-out font-sans ${isSearchOpen ? "w-32 md:w-48 opacity-100 pr-8" : "w-0 opacity-0 pointer-events-none"}`}
      />
      <Search 
        size={24} 
        onClick={() => setIsSearchOpen(!isSearchOpen)} 
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      gsap.set(menuRef.current, { display: "block" });
      const tl = gsap.timeline();
      tl.to(menuRef.current, { x: 0, duration: 0.8, ease: "expo.out" })
        .fromTo(".menu-item", { x: -30, opacity: 0 }, { x: 0, opacity: 1, stagger: 0.1, duration: 0.5, ease: "power3.out" }, "-=0.4");
    } else {
      gsap.to(menuRef.current, { x: "-100%", duration: 0.6, ease: "expo.in", onComplete: () => { gsap.set(menuRef.current, { display: "none" }); } });
    }
  }, [isOpen]);
  ...
  return (
    <nav className="absolute top-0 w-full z-50 bg-transparent px-6 lg:px-12 py-8">
      <div className="grid grid-cols-3 items-center w-full max-w-[1800px] mx-auto">
        ...
        {/* LEFT: TOGGLE (RESTORED) */}
        <div className="flex justify-start">
          <button onClick={() => setIsOpen(true)} className="flex items-center gap-3 group focus:outline-none cursor-pointer">
            <div className="relative w-6 h-5 flex flex-col justify-between">
              <span className="w-full h-[1.5px] bg-black transition-all group-hover:w-1/2"></span>
              <span className="w-full h-[1.5px] bg-black"></span>
              <span className="w-full h-[1.5px] bg-black transition-all group-hover:w-3/4"></span>
      ...
      {/* FULL SCREEN OVERLAY (RESTORED) */}
      <div ref={menuRef} className="fixed inset-0 bg-[#f8f8f8] z-[9999] overflow-hidden shadow-2xl" style={{ display: "none", transform: "translateX(-100%)" }}>
        <div className="p-8 md:p-16 flex flex-col h-full max-w-7xl mx-auto text-black">
          <div className="flex justify-between items-center">
            <span className="text-sm tracking-[0.4em] font-light">DMA STUDIO</span>
            <button onClick={() => setIsOpen(false)} className="group p-4 flex items-center gap-2 hover:opacity-60 transition-opacity cursor-pointer">
              <span className="text-[10px] uppercase tracking-widest">Close</span>
              <X size={24} className="stroke-[1px]" />
            </button>
          </div>
          ...
          <div className="mt-12 md:mt-20 grid grid-cols-1 md:grid-cols-2 gap-10 flex-1 overflow-y-auto">
            <div className="flex flex-col gap-2 md:gap-4">
              {navLinks.map((link, index) => (
                <Link key={index} href={link.path} onClick={() => setIsOpen(false)} className="menu-item group flex items-center justify-between text-4xl md:text-7xl font-light text-black border-b border-transparent py-2 transition-all">
                  <span className="relative flex items-baseline">
                    {link.name}
                    {link.badge && <span className="ml-3 text-[9px] leading-none md:absolute md:ml-0 md:top-14 md:-right-22 md:text-[12px] text-[#be1e2d] font-bold tracking-tighter uppercase whitespace-nowrap">{link.badge}</span>}
                  </span>
            ...
            <div className="hidden md:flex flex-col justify-center border-l border-gray-100 pl-20">
              <p className="text-xs uppercase tracking-[0.3em] text-gray-400 mb-4">Featured Collection</p>
              <div className="aspect-[3/4] w-72 relative overflow-hidden shadow-2xl bg-gray-200">
                {isMounted && <Image src={HeroImage2} alt="Featured" fill className="object-cover" />}
              </div>
            </div>
          </div>
