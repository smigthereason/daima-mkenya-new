...
const SearchLogic = ({ isSearchOpen, setIsSearchOpen, searchQuery, setSearchQuery, handleSearchSubmit }: any) => {
  ...
  useEffect(() => {
    // Only auto-open on desktop if we are on the search page
    // We check window width to prevent the mobile overlay from popping up automatically
...
const Navbar = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      // CRITICAL: Close the state immediately so the overlay unmounts
      setIsSearchOpen(false);
      // Then navigate
      router.push(`/search?q=${encodeURIComponent(searchQuery)}`);
    }
