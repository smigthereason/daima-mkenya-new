...
export default function ProductCard({
  ...
  const handlePurchaseClick = () => {
    if (onPurchase) {
      ...
      // Map 'label' from ProductColor to the 'name' property expected by Checkout
      const color = {
        name: colorObj.label,
        hex: colorObj.hex,
      };

      onPurchase(activeProduct, size, color);
    }
  };
      ...
      <div className="absolute top-6 left-1/2 -translate-x-1/2 xl:left-auto xl:translate-x-0 xl:top-10 xl:right-12 z-30 flex items-center gap-8">
        <div className="flex items-center gap-8 bg-white/60 backdrop-blur-md px-6 py-3 rounded-full border border-gray-200/50 shadow-sm">
          <button
            ...
            className="hover:opacity-30 transition-opacity p-1 group cursor-pointer"
          >
            <ChevronLeft size={24} className="font-medium" />
          </button>
          <span className="text-[14px] tracking-[0.5em] text-gray-500 font-light whitespace-nowrap">
        ...
        <div className="max-w-sm mx-auto xl:ml-0 w-full">
              ...
              <div className="flex gap-4 sm:gap-6">
                {activeProduct.colors.map((c, i) => (
                  <button
                    key={i}
                    onClick={() => setSelectedColor(i)}
                    className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full transition-all duration-500 ring-offset-4 sm:ring-offset-8 ring-1 ${
                      i === selectedColor
              ...
              <div className="grid grid-cols-4 gap-2 sm:gap-3">
                {activeProduct.sizes.map((s, i) => (
                  <button
                    key={s}
                    onClick={() => setSelectedSize(i)}
                    className={`py-4 sm:py-5 text-[14px] sm:text-[16px] tracking-[0.2em] font-medium transition-all duration-500 border ${
                      i === selectedSize
          ...
          <div className="mt-16 md:mt-20 space-y-4">
            <button
              ...
              className="group relative w-full overflow-hidden border-2 border-black bg-black py-6 sm:py-7 text-[12px] sm:text-[14px] font-black uppercase tracking-[0.4em] text-white transition-colors duration-500 shadow-lg cursor-pointer"
            >
              <span className="absolute inset-0 z-0 translate-y-full bg-white transition-transform duration-500 ease-out group-hover:translate-y-0" />
              <span className="relative z-10 group-hover:text-black">
                Purchase Item
