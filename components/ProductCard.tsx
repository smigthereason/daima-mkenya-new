...
//   // Fixed: Only reset feedback when color/size changes AND it's different from last added config
//   useEffect(() => {
//     if (lastAddedConfig) {
//       const isSameConfig =
...
//       // Only reset feedback if the configuration changed AND it's not the one we just added
...
//       // Keep feedback showing for at least 3 seconds, but don't auto-hide
//       // It will only hide when user changes color/size
//     } catch (error) {
...
//             <div className="border-8 border-red-600 rotate-[-15deg] px-12 py-4 rounded-sm opacity-70">
...
//         {/* Fixed: Show feedback message and keep it visible */}
//         {addedFeedback && !stockStatus.disabled && (
//           <div className="mt-4 space-y-2">
//             <p className="text-[10px] text-green-600 uppercase tracking-wider text-center font-bold animate-pulse">
//               ✓ Item successfully added to your bag!
//             </p>
//             <p className="text-[9px] text-gray-400 uppercase tracking-wider text-center">
//               Change color or size to update selection
//             </p>
//           </div>
//         )}
...
export default function ProductCard({ productSlug }: ProductCardProps) {
  ...
  return (
    <div
      ...
      className="flex flex-col xl:flex-row w-full bg-white relative min-h-screen text-black antialiased overflow-x-hidden mt-0 border-t border-neutral-400"
    >
      {/* 1. Main Stage Viewport Section (Appears first on mobile viewports matching standard luxury e-comm) */}
      <div className="w-full xl:w-[44%] bg-[#F5F5F5] relative flex items-center justify-center h-[55vh] sm:h-[65vh] xl:h-screen overflow-hidden group border-b xl:border-b-0 xl:border-r border-neutral-200">
        <div
          className={`relative w-full h-full transition-all duration-300 ease-in-out
            ${isSwapping ? "opacity-0 scale-95 blur-sm" : "opacity-100 scale-100 blur-0"}`}
        >
      ...
      {/* 2. Unified Gallery Grid + Narrative Section */}
      <div className="w-full xl:w-[28%] p-6 sm:p-8 md:p-12 xl:p-16 border-b xl:border-b-0 xl:border-r border-neutral-200 bg-white flex flex-col justify-between">
        <div>
          ...
          <div className="grid grid-cols-4 xl:grid-cols-3 gap-3 xl:gap-4 mb-8">
            {galleryImages.map((img, i) => {
              const isCenterImage = img === centerImage;
              return (
                <button
      ...
      {/* 3. Details, Interactive Swatches & Purchase Processing Controls */}
      <div className="w-full xl:w-[28%] p-6 sm:p-8 md:p-12 xl:p-16 bg-white flex flex-col justify-center">
