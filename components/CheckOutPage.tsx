...
//                   {/* PesaPal Option - Only Option */}
//                   <div className="p-5 md:p-7 border-2 border-black bg-white shadow-lg">
...
//                       </div>
//                     </div>
//                     <div className="mt-4 bg-black px-3 py-1 text-[8px] font-bold text-white uppercase rounded w-fit">
//                       Secure Payment
//                     </div>
//                   </div>
//                 </section>
...
export default function CheckOutPage({ onBack }: CheckOutPageProps) {
  ...
  const handleCompletePurchase = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // IMPORTANT: data.redirect_url must be the valid V3 URL returned by PesaPal
      if (data.redirect_url) {
        if (window.PesaPal) {
          // This triggers the modal overlay
          window.PesaPal.pay(data.redirect_url);

          // Reset loading state so if they close the modal, they can try again
          // The modal stays in front of this page
          setIsProcessing(false);
        } else {
          // Fallback if the script isn't loaded (e.g. adblockers)
          window.location.href = data.redirect_url;
        }
      } else {
        alert(`Error: ${data.message || "Could not initiate payment."}`);
  ...
  return (
    <div className="min-h-screen bg-white text-black antialiased mt-20 md:mt-32 border-t border-neutral-200">
      {/* ── PESAPAL V3 SDK ── */}
      {/* Use https://pay.pesapal.com/v3/js/pesapal.js for Production */}
      <Script
              ...
              >
                ...
                <div className="pt-8">
                  <button
                    type="submit"
                    disabled={isProcessing || cartItems.length === 0}
                    className="group relative w-full overflow-hidden bg-black py-7 md:py-9 text-[13px] md:text-[15px] font-black uppercase tracking-[0.6em] text-white transition-all duration-500 disabled:opacity-50 border border-transparent hover:border-black"
                  >
                    {/* White background overlay that slides up on hover/click */}
                    <span className="absolute inset-0 z-0 translate-y-full bg-white transition-transform duration-500 ease-out group-hover:translate-y-0" />
