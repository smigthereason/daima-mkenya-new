    ...
    const fetchProducts = async () => {
      try {
        ...
        const formattedItems = productsArray.map((product) => {
          ...
          else if (Assets[`Model${product.id}` as keyof typeof Assets]) {
            imageSource = (Assets[`Model${product.id}` as keyof typeof Assets] as any).src || Assets[`Model${product.id}` as keyof typeof Assets];
          } 
          // Default Fallback
          else {
            imageSource = Assets.Model1.src || Assets.Model1;
          }
